...
def parse_markdown_template(md: str) -> List[Dict[str, Any]]:
    ...
    for line in lines:
        ...
        if line.strip().startswith("```"):
            in_code = not in_code
            if current: current["placeholder"] += line + "\n"
            continue
...
def analyze_logic_node(state: State) -> Dict[str, Any]:
    ...
    target_sections = [
        n for n in state["flat"]
        if (not n["children"] or n["placeholder"].strip()) and 
           (n["instructions"].strip() or n["placeholder"].strip())
    ]
    
    writing_order = determine_writing_order(target_sections, logical_analysis)
    
    logger.info(f"确定写作顺序：共 {len(writing_order)} 个章节")
...
def determine_writing_order(sections: List[Dict[str, Any]], analysis: str) -> List[int]:
    """根据逻辑分析确定写作顺序"""
    sections_with_priority = []
    
    for section in sections:
        priority = section["level"] * 1000 + section["id"]
        sections_with_priority.append((priority, section["id"]))
    
    sections_with_priority.sort()
    return [section_id for _, section_id in sections_with_priority]
...
def sequential_write_node(state: State) -> Dict[str, Any]:
    ...
    try:
        ...
        content = safe_llm_invoke(prompt, {
            "title": node["title"],
            "parent_chain": parent_chain_titles(state["tree"], current_section_id),
            "level": level_info,
            "instructions": (node["instructions"] or "").strip(),
            "placeholder": (node["placeholder"] or "").strip(),
            "logical_analysis": state["logical_analysis"],
            "completed_sections": state["completed_sections"][-1000:] if state["completed_sections"] else "",  # 只传递最近的内容，避免过长
            "length_clause": length_clause,
            "config_str": config_str[:500],  # 限制配置信息长度
            "领域": domain_str,
        })
